...
{
    ui->setupUi(this);
    ui->lbl_graph->setPicture(this->graph);
//    GraphDraw(this->graph);
...
void MainWindow::loadData()
{
    ...
    try {
        ...
        this->colRegNumber = fi.colRegNumber;
        this->colYearNumber = fi.colYearNumber;
    }
...
void MainWindow::calcMetrics()
{
    ...
    try {
        TableGetColumnData(*this->ui->table_display, io, this->ui->le_column->text(), this->colYearNumber);
    }
    catch (std::exception& e) {
        QMessageBox::critical(this, "Расчет не удался", e.what());
    }

    CSVHelperFrontController(METRICS_FILTER, nullptr, &io);
